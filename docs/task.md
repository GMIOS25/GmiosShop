...
- `[ ]` Phase 2: SePay & Resend Edge Functions
  ...
  - `[ ]` **Task 5**: Triển khai Edge Function `sepay-webhook` nhận kết quả giao dịch từ SePay và gửi Email qua Resend.
- `[ ]` Phase 3: Angular Infrastructure
  - `[ ]` **Task 6**: Cài đặt Supabase JS SDK, viết `SupabaseService`, `AccountService` và `OrderService` sử dụng Signals.
